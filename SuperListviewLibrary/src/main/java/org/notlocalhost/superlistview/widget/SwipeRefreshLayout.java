...
public class SwipeRefreshLayout extends ViewGroup {
    ...
    private SwipeHeaderView mHeaderView;
    ...
    private SwipeHeaderView.SwipeHeaderType mHeaderType = SwipeHeaderView.SwipeHeaderType.SPINNER;
    ...
    public SwipeRefreshLayout(Context context, AttributeSet attrs) {
        ...
        mHeaderView = new SwipeHeaderView(this);
    ...
    private void setHeaderOffset(int offset) {
        mHeaderView.setHeaderOffset(offset);
    }
    ...
    private void ensureTarget() {
        ...
        if (mTarget == null) {
            ...
            mTarget = getChildAt(TARGET_POSITION);
    ...
    public void draw(Canvas canvas) {
        super.draw(canvas);
        if(mHeaderType == SwipeHeaderView.SwipeHeaderType.DEFAULT) {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        final DisplayMetrics metrics = getResources().getDisplayMetrics();
        mRefreshHeaderHeight = (int) Math.min(
                ((View) getParent()) .getHeight() * MAX_SWIPE_DISTANCE_FACTOR,
                REFRESH_TRIGGER_DISTANCE * metrics.density);
        mProgressBar.setBounds(0, 0, width, mProgressBarHeight);
        mHeaderView.setBounds(0, 0, width, mRefreshHeaderHeight);
        if (getChildCount() == 0) {
            return;
        }
        final View child = getChildAt(TARGET_POSITION);
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (getChildCount() > TARGET_POSITION) {
            getChildAt(TARGET_POSITION).measure(
        ...
        if (android.os.Build.VERSION.SDK_INT < 14) {
            if (mTarget instanceof AbsListView) {
                ...
                return absListView.getChildCount() > 0
                        && (absListView.getFirstVisiblePosition() > 0 || absListView.getChildAt(TARGET_POSITION)
    ...
    private void startRefresh() {
        if(!mRefreshing) {
            removeCallbacks(mCancel);
            if ((mHeaderType != SwipeHeaderView.SwipeHeaderType.SPINNER)
                    && (mHeaderType != SwipeHeaderView.SwipeHeaderType.SPINNER_WITH_TEXT)) {
                mReturnToStartPosition.run();
            }
            setRefreshing(true);
            mListener.onRefresh();
    ...
    private void setTargetOffsetTopAndBottom(int offset) {
        ...
        setHeaderOffset(mCurrentTargetOffsetTop);
    }
    ...
    public void setHeaderText(int resId) {
        try {
            ...
            throw new Resources.NotFoundException("You must use a valid resource id. resId: " + resId);
        }
    }
